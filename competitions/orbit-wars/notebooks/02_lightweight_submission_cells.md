# 02 Lightweight Orbit Wars Submission Cells

Use this version if the Kaggle Notebook or browser becomes slow or freezes.

This avoids `env.render(...)` and only does the minimum needed to create and validate `main.py`.
...
## Cell 3: Smoke Test Without Rendering
...
print("main.py is ready for submission")
```

## Cell 4: Confirm File Exists
...
Use Kaggle's `Submit to competition` button and select `main.py`.
...
Do not run `env.render(...)` before the first submission.
